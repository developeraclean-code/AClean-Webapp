-- 181 — Rekap pekerjaan aktual bulanan untuk Dashboard Owner/Admin.
--
-- Sumber kebenaran adalah unit pada service_reports VERIFIED, bukan qty rencana
-- order. Semua agregasi dilakukan PostgreSQL; browser hanya menerima belasan angka.
...
BEGIN
  ...
  WITH month_reports AS (
    ...
    CROSS JOIN LATERAL (
      ...
      -- Laporan legacy kadang hanya memiliki total_units. Tetap hitung total
      -- cleaning-nya, tetapi masukkan tipe AC ke kelompok belum dikategorikan.
    ...
    SELECT n.*,
           ...
           (n.work_text ~ 'pemasangan unit|pasang (unit )?(indoor|outdoor)') AS is_installation,
...
COMMENT ON FUNCTION public.get_dashboard_monthly_work_summary(date) IS
  'Agregasi unit pekerjaan aktual per bulan dari service_reports VERIFIED untuk Dashboard Owner/Admin.';
