...
export default async function handler(req, res) {
  ...
  if (route === "send-wa" && req.method === "POST") {
    ...
    if (!token) {
      console.warn("FONNTE_TOKEN tidak diset — WA tidak terkirim");
      return res.status(200).json({ success: false, error: "FONNTE_TOKEN belum diset di Vercel env" });
    }

    try {
      // Fonnte API wajib pakai form-data (bukan JSON)
