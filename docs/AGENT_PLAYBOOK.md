...
| Menyusun URL foto R2 sendiri (r2.dev / endpoint S3 / `/api/foto?key=`) | Pakai `fotoUrl()` di `src/lib/fotoUrl.js` — SATU sumber kebenaran; `fotoSrc()` di App.jsx/portal/MatTrack/Project cuma pembungkusnya (dulu 5 salinan berbeda, gampang lupa satu). Ia yang memilih CDN (`VITE_R2_CDN_URL`) vs proxy, dan sengaja menahan `.pdf`/`.html` tetap di proxy. **KOREKSI 1 Sep 2026:** klaim lama "bucket non-publik, wajib proxy" itu SALAH — public access R2 aktif (`R2_PUBLIC_URL` dipakai kirim PDF ke Fonnte) dan route `foto` ada di `PUBLIC_ROUTES` (`api/[route].js:14`), jadi proxy BUKAN lapisan keamanan. Alasan asli proxy dipakai: r2.dev di-rate-limit Cloudflare. Pengaman foto = nama berkas hash yang tak bisa ditebak — jangan simpan berkas rahasia di bucket ini |
